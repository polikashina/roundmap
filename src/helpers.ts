...
export const downloadPdf = (svg: SVGSVGElement | null) => {
  ...
  convertSVGtoJPEG(blobURL, width, height, (jpeg: string) => {
    ...
    doc.addImage(jpeg, "JPEG", 10, 40, width / 2, height / 2);
    doc.save(`balance-wheel-${date}.pdf`);
  });
};

const convertSVGtoJPEG = (blobURL: string, width: number, height: number, cb: (jpeg: string) => void) => {
